...
export default function CorporateActionPortal() {
  ...
  const handleIsoMessageSubmit = (type) => {
    const message = type === "ISO15022" ? iso15022Message : iso20022Message;
    if (message) {
      setSubmittedData([...submittedData, { eventID: type, issuer: "Message", eventTitle: message, recordDate: "N/A", paymentDate: "N/A", dataHash: "N/A", status: "Pending Validation" }]);
      type === "ISO15022" ? setIso15022Message("") : setIso20022Message("");
    }
  };
        ...
        <TabsContent value="data-user">
          <Card className="mb-4">
            <CardContent className="p-4">
              ...
              <Button onClick={handleApiSubmit} className="mb-4">Submit via API</Button>
              <Input placeholder="Enter ISO 15022 Message" value={iso15022Message} onChange={(e) => setIso15022Message(e.target.value)} className="mb-2" />
              <Button onClick={() => handleIsoMessageSubmit("ISO15022")} className="mb-4">Submit via ISO 15022</Button>
              <Input placeholder="Enter ISO 20022 Message" value={iso20022Message} onChange={(e) => setIso20022Message(e.target.value)} className="mb-2" />
              <Button onClick={() => handleIsoMessageSubmit("ISO20022")} className="mb-4">Submit via ISO 20022</Button>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
